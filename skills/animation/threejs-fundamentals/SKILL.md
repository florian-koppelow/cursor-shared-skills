...
description: Core concepts of Three.js for building interactive 3D web experiences. Covers scene setup, cameras, renderers, and object management.
---
...
- Setting up the initial boilerplate for a new Three.js project
- Implementing responsive camera controls and renderer adjustments
- Organizing complex 3D models using `Object3D` groups
- Debugging issues related to object positioning, scaling, or rotation

## Pro Tips

- Start with `AmbientLight` and a simple `DirectionalLight` for basic scene illumination
- Utilize `Object3D` for grouping related meshes, lights, or other groups
- Always use `renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))` to avoid overstressing lower-end GPUs
